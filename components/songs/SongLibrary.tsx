...
export default function SongLibrary({ 
  ...
  // Obtener tutoriales del backend
  const { songs: backendSongs, loading, error, refetch } = useSongsFromBackend();

  // Combinar tutoriales del backend con tutoriales locales y custom
  const allSongs = useMemo(() => {
    const fallbackSongs = error ? sampleSongs : []; // Solo usar tutoriales locales si hay error
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              {allSongs.length} tutoriales disponibles
            ...
            {error && (
              <div className="flex items-center text-amber-400 text-sm mt-1">
                ...
                Modo offline (usando tutoriales locales)
              </div>
            )}
      ...
      {filteredSongs.length === 0 && (
        <div className="text-center py-12 bg-black/30 backdrop-blur-md rounded-xl border border-gray-800">
          <p className="text-gray-400">
            No se encontraron tutoriales que coincidan con los filtros.
          </p>
